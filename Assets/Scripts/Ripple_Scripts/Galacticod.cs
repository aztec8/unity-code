...
public class Galacticod : MonoBehaviour {
	...
	{
		// GALACTICOD ENGAGE
		galacticod = GameObject.Find("Clown_Fish_Character");
...
//		Debug.Log ("Gesture count: "+swipeCounter);
	...
	private void accelerate(){
		Vector3 accelForce = transform.forward * 10f;
	}
	...
	private void ProccessInput()
	{
		...
		if (Input.GetKey ("up"))
		{
			...
			transform.Rotate(Vector3.left*TURN_SPEED/2,Space.Self);
		}
		// down
		if (Input.GetKey ("down"))
		{
			//transform.Translate(down);
			transform.Rotate(Vector3.right*TURN_SPEED/2,Space.Self);
		}
